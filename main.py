...
def wave():
    ...
    for i, j, k in ti.ndrange((-64, 64), (-64, 64), (-64, 64)):
        ...
        if k < h:#wave()
            scene.set_voxel(vec3(i, k, j), 1, vec3(0.33, 0.71, 0.1))
        elif -64 < k < -52:#lake
            ...
            if tree_count >= 0 and i % 15 == 0 and j % 15 == 0 and -60 < i < 60 and -60 < j < 60:#tree
                build_tree((i + ((ti.random(ti.i32) % 10) - 5) , h, j + ((ti.random(ti.i32) % 10) - 5)), 1, vec3(ti.random(ti.f32), ti.random(ti.f32), ti.random(ti.f32)))
            # if i == 0 and j == 0:
