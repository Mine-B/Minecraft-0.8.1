...
def wave():
    ...
    for i, j, k in ti.ndrange((-64, 64), (-64, 64), (-64, 64)):
        h = 4 * ti.sin(ti.cast(i - 64, ti.f32) / (64 * 4) * 3.14) * \
            ti.sin(ti.cast(j + 64, ti.f32) / (64 * 4) * 3.14) - 50
        min_height = min(ti.cast(h, ti.i32), min_height)
        if k < h:#wave()
            scene.set_voxel(vec3(i, k, j), 1, vec3(0.10, 0.44, 0.24))#0.33, 0.71, 0.1
        elif -64 < k < -52:#lake
            scene.set_voxel(vec3(i, k, j), 1, \
                            vec3(0.26, ((ti.random(ti.i32) % 50) + 100) / 255, 1.00))
        elif h <= k < h + 1:
            if i == 10 and j == 10:
                steve((i, k, j))
            if light_count >= 0 and ti.random(ti.i32) % 100000 >= 99650:#light block
                scene.set_voxel(vec3(i, k, j), 2, \
                                vec3(ti.random(ti.f32), ti.random(ti.f32), ti.random(ti.f32)))
                light_count -= 1
            if tree_count >= 0 and i % 15 == 0 and j % 15 == 0 and -60 < i < 60 and -60 < j < 60:#tree
                build_tree((i + ((ti.random(ti.i32) % 10) - 5), h, j + ((ti.random(ti.i32) % 10) - 5)), \
                           1, vec3(0.33, 0.71, 0.1))
